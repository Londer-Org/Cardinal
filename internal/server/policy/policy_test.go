...
// repoRoot finds the tree this test lives in, by walking up to go.mod.
//
// Rather than counting "../.." from here. That count is a hidden dependency on
// where this package sits, and it broke silently the moment the package moved
// one level deeper — in a way the compiler cannot see, because a wrong path is
...
func repoRoot(t *testing.T) string {
	...
	for {
		...
		if parent == dir {
			t.Fatal("no go.mod above this test; cannot find the repository root")
		}
		dir = parent
	}
}
