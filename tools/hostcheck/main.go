...
func main1() int {
	...
	// The real writer, against the real path, so what sshd is asked to honour
	// is the file the agent produces rather than one written to please it.
	// The authority is generated by the script and handed in here.
	if userCA := os.Getenv("CARDINAL_USER_CA"); userCA != "" {
		a := &agent.Agent{UserCAPath: "/etc/ssh/cardinal_user_ca.pub"}
		if _, installErr := a.InstallUserCAKeys([]string{userCA}); installErr != nil {
			fmt.Fprintln(os.Stderr, installErr)
			return 1
		}
	}
