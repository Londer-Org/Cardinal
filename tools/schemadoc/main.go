...
func run() error {
	...
	out, err := os.Create("docs/schema.md")
	if err != nil {
		return err
	}
	write(out, tables)
...
func write(out *os.File, tables map[string]*table) {
	...
	_, _ = fmt.Fprint(out, strings.ReplaceAll(`# Database schema
...
func writeDiagram(out *os.File, tables map[string]*table, names []string) {
...
func writeTable(out *os.File, t *table) {
	fmt.Fprintf(out, "### `%s`\n\n", t.Name)
	if t.Comment != "" {
		fmt.Fprintf(out, "%s\n\n", t.Comment)
	}
	if len(t.Partitions) > 0 {
		...
		fmt.Fprintf(out, "Partitioned by time: `%s`. Retention is a partition drop "+
			"rather than a delete.\n\n", strings.Join(t.Partitions, "`, `"))
	}

	fmt.Fprintln(out, "| Column | Type | Null | Default | Notes |")
	fmt.Fprintln(out, "|---|---|---|---|---|")
	for _, c := range t.Columns {
		null := "no"
		if c.Nullable {
