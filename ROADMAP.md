...
| SSF has no stream management API | ~~A receiver that expects to poll cannot~~ **Poll delivery is closed.** A receiver now collects with `-delivery poll` and a credential of its own (RFC 8936), which is what a receiver behind NAT or without a public address needs. Creating its own stream over the API it still cannot — the configuration document says so rather than leaving it to be discovered | 5 |
| ~~SSF streams are configured only from the CLI~~ | **Closed.** Integrations › Security events lists every receiver, what it subscribes to, whether delivery is failing, and the issuer and JWKS a receiver author needs. Behind ManageApplications, because a stream belongs to an application | done |
